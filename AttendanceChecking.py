...
class AttendanceChecking:
    ...
    * Creating new or modify existing AttendanceChecking.xlsx file with the predefined format
    ...
    def new_standard_file(file_path):
        ...
        new_sheet.cell(4, 4).value = "1 = absent"
        new_sheet.cell(4, 4).font = AttendanceChecking.cell_format("header")

        for col in range(AttendanceChecking.start_table_cell[1], len(AttendanceChecking.header_list) + 1):
            new_sheet.cell(AttendanceChecking.start_table_cell[0], col).value = \
                AttendanceChecking.header_list[col - AttendanceChecking.start_table_cell[1]]
            AttendanceChecking.format_font(new_sheet, "header", [AttendanceChecking.start_table_cell[0], col])
            new_sheet.cell(AttendanceChecking.start_table_cell[0], col).border = AttendanceChecking.cell_format("thin_border")
    ...
    def if_standard_excel(self):
        ...
        if (id == -1) or (last_name == -1) or (first_name == -1) or (group == -1) \
                or (total == -1) or (next_total is not None):
            return False
        else:
            for col in range(group[1] + 1, total[1]):
                ...
                if cell_type == str:
                    try:
                        cell_type = type(parser.parse(self.sheet.cell(id[0], col).value))
                        if cell_type != datetime.datetime:
                            print(
                                "Cell must be a date: " + str([id[0], col]) + " - " + str(
                                    self.sheet.cell(id[0], col).value)
                                + " is a " + str(cell_type))
                            return False
                    except ValueError:
                        print(
                            "Unknown string format: " + str([id[0], col]) + " - " +
    ...
    def update_total_col(self, from_row, to_row, from_col, to_col, row_total_header, col_total_header):
        for row in range(row_total_header, to_row - from_row + 1):
            sum_func = "=COUNTBLANK(" + str(self.get_col_letter(from_col)) + str(row) + ":" + \
                       str(self.get_col_letter(to_col)) + str(row) + ")"
    ...
    def start_checking(self, mssv_array=[]):
        ...
        * Execute attendance checking for each id in ID column
        ...
        if not self.if_exist_table_header(self.get_current_date(), [col_total[0], col_total[1] - 1]):
        ...
        for id in mssv_array:
            ...
            if checked:
                ...
                self.update_total_col(col_total[0], self.sheet.max_row, group[1] + 1,
                                      new_checking[1], new_checking[0] + 1, new_checking[1] + 1)
